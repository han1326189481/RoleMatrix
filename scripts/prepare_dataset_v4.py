...
DST = Path(r"D:\RoleMatrix\data\lora_xiaor_real_v4\img")
DST.mkdir(parents=True, exist_ok=True)

# v4 修正（依据专业视觉分析）：
# - 去 slightly chubby（导致脸胖）→ slender soft face（脸瘦）+ petite（娇小）
# - 眼镜锁定型号：black thick-rimmed rectangular glasses（粗黑框矩形，仅戴眼镜的图补充）
# - 发型：shaggy layered hair（凌乱层次长发）
# - 神态：calm cool vibe（清冷克制氛围，统一追加）
FIXED = (
    "little_r, young east asian woman, slender soft face, soft oval face, baby face, "
    "shaggy layered hair, wispy bangs, fair skin, petite"
)
GLASSES = "black thick-rimmed rectangular glasses"
...
CAPTIONS: dict[tuple[str, str], tuple[str, bool, bool]] = {
    ...
    ("全身照", "1.jpg"): ("full body, oversized sweatshirt, black pleated skirt, black shoulder bag, white socks, indoor living room, warm lighting, standing, realistic", False, True),
    ("全身照", "2.jpg"): ("full body, oversized white sweatshirt, black pleated skirt, black shoulder bag, white socks, outdoor by water, daylight, side view, realistic", False, True),
    ("全身照", "3.jpg"): ("full body, oversized sweatshirt, black shorts, sneakers, outdoor waterfront, sunset, natural light, realistic", False, True),
    ("全身照", "4.jpg"): ("oversized beige hoodie with bold black letters, black pleated skirt, convenience store, indoor, front view, full body, realistic", False, True),
    ("全身照", "5.jpg"): ("white hoodie with bold black letters, black pleated skirt, night street, street lights, full body, realistic", False, True),
    ("全身照", "6.jpg"): ("white hoodie with bold black letters, dark pleated skirt, striped socks, indoor dim lighting, full body, realistic", False, True),
    ("全身照", "7.jpg"): ("full body, white hoodie with bold black letters, black pleated skirt, night city street, tall buildings lights, realistic", False, True),
    ("全身照", "9.jpg"): ("sitting on stairs, white hoodie with bold black letters, dark pleated skirt, knee socks, chunky sneakers, indoor dim lighting, realistic", False, True),
    ...
    ("正脸照", "2.jpg"): ("front portrait, looking at camera, white hoodie with black print, indoor, plush toys and shelf background, warm lighting, realistic", True, True),
    ...
    ("生活照", "1.jpg"): ("holding phone, light top, cafe, window natural light, side view, candid, realistic daily life", True, True),
    ("生活照", "2.jpg"): ("black and white photo, loose white top, black bag, bookstore, bookshelf background, side view, candid, realistic", True, True),
    ("生活照", "3.jpg"): ("looking at phone, white hoodie, black pleated skirt, black backpack, night street, city lights, candid, realistic", True, True),
    ("生活照", "4.jpg"): ("browsing groceries, white shirt and dark cardigan, crossbody bag, grocery store, side view, candid, realistic daily life", True, True),
    ("生活照", "6.jpg"): ("sitting on subway, white sweater and dark vest, other passengers behind, side view, candid, realistic daily life", True, True),
    ("生活照", "8.jpg"): ("ponytail, white top, black pleated skirt, dark tote bag, cafe counter, side profile, slightly overexposed, film photo style, realistic", True, True),
}
...
# 小R 设定：除洗澡/运动外永远戴黑框矩形眼镜 —— 全部样本统一戴眼镜，锁定签名特征
FACE_CROPS = [
...
def _caption(tail: str, has_glasses: bool) -> str:
    ...
    # 统一追加清冷克制神态（依据专业分析：基底图表情清冷/无感/微微厌世）
    return f"{FIXED}{g}, {tail}, calm cool vibe"
